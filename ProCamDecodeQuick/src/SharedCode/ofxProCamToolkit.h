...
void grayDecode(string path, cv::Mat& binaryCoded, cv::Mat& cam, GrayCodeMode mode, cv::Mat& mask);

//void thresholdedToBinary(vector<cv::Mat>& thresholded, cv::Mat& binaryCoded);
void thresholdedToBinary(vector<cv::Mat>& thresholded, cv::Mat& binaryCoded, const cv::Mat& mask);
...
void getProCamImages(string path, C& cam, P& pro, int width, int height, GrayCodeMode mode) {
    ofLog()<<"getProCamImages() path "<<path<<" width "<<width<<" height "<<height;
