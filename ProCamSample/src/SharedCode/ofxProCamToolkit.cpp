...
//	vector<ofFloatColor>& colors = cloud.getColors();
    vector<ofColor> colors = cloud.getColors();
	vector<glm::vec3> surface = cloud.getVertices();
	for(int i = 0; i < surface.size(); i++) {
		if (surface[i] != zero) {
			...
			if(colors.size() > 0) {
//				unsigned char color[3] = {unsigned char(colors[i].r * 255), unsigned char(colors[i].g * 255), unsigned char(colors[i].b * 255)};
                unsigned char color[3] = cloud.getColor(i); //{colors[i].r, colors[i].g, colors[i].b};
