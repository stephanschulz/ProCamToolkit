#include "ofApp.h"
//https://forum.openframeworks.cc/t/procamtoolkit-procamdecode-projector-pixel-positions/8847
//https://www.instructables.com/Structured-Light-3D-Scanning/
...
void thresholdedToBinary(ofShortImage& binaryCoded, vector<Mat>& thresholded) {
	...
	ofPixels binaryCodedPixels = binaryCoded.getPixels();
...
void buildBinaryCoded(ofShortImage& binaryCodedX, ofShortImage& binaryCodedY, ofImage& binaryCoded) {
    ...
    ofPixels xPixels = binaryCodedX.getPixels();
    ofPixels yPixels = binaryCodedY.getPixels();
...
ofVec2f cameraToProjector(ofVec2f cameraPosition) {
    ...
    ofPixels xPixels = curBinaryCodedX->getPixels();
    ofPixels yPixels = curBinaryCodedY->getPixels();
...
void saveRemap(ofShortImage& binaryCodedX, ofShortImage& binaryCodedY, string filename) {
	...
	ofFloatPixels remapPixels = remap.getPixels();
	for(int i = 0 ; i < tn; i++) {
...
//    ofSetDataPathRoot(ofToDataPath());
	/*
	panel.setup();
	...
	*/
	...
	if(false || !(binaryCodedLeftX.load(base + "left/vertical.png") &&
								binaryCodedLeftY.load(base + "left/horizontal.png") &&
								binaryCodedRightX.load(base + "right/vertical.png") &&
								binaryCodedRightY.load(base + "right/horizontal.png"))) {
		...
		binaryCodedLeftX.save(base + "left/vertical.png");
		binaryCodedLeftY.save(base + "left/horizontal.png");
		binaryCodedRightX.save(base + "right/vertical.png");
		binaryCodedRightY.save(base + "right/horizontal.png");
	}
...
void addQuad(ofMesh& quads, ofColor color, ofVec2f topLeft, ofVec2f size) {
	...
	quads.addColor(cur);
    ofVec2f temp_v = cameraToProjector(topLeft + ofVec2f(0,0));
    glm::vec3 temp_g = glm::vec3(temp_v.x , temp_v.y, 0);
	quads.addVertex(temp_g);
    
	quads.addColor(cur);
    temp_v = cameraToProjector(topLeft + ofVec2f(0,size.y));
    temp_g = glm::vec3(temp_v.x , temp_v.y, 0);
	quads.addVertex(temp_g);
    
	quads.addColor(cur);
    temp_v = cameraToProjector(topLeft + ofVec2f(size.x,0));
    temp_g = glm::vec3(temp_v.x , temp_v.y, 0);
	quads.addVertex(temp_g);
	...
	quads.addColor(cur);
    temp_v = cameraToProjector(topLeft + ofVec2f(size.x, size.y));
    temp_g = glm::vec3(temp_v.x , temp_v.y, 0);
    quads.addVertex(temp_g);
}
...
void ofApp::draw() {
	...
	leftLut.setSource(leftFbo.getTexture());
	leftLut.update();
	...
	rightLut.setSource(rightFbo.getTexture());
	rightLut.update();
